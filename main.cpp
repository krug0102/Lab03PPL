...
// A void function that takes an array of tempData and changes it in place to convert all temperatures to Celsius
void convertToCelsius(tempData arr[], int size) {
    for (int i = 0; i < size; i++) {
        arr[i].temp = fToC(arr[i].temp);
    }
}
...
// Task 3 (6 points)
// Write a void function that takes an array of tempData and changes it in place to convert all temperatures to Celsius.
// Print out the array before and after the call.
    ...
    printTempData(data, 5);

    convertToCelsius(data, 5);
